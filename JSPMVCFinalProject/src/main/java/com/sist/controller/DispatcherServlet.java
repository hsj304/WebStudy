 ...
 *  			자바와 HTML을 분리하는 이유
 *  			---
 *  			확장성, 재사용, 변경이 쉽다 (JSP는 한번 사용하면 버린다(일회용))
 *  			MVC , MVVP ....
 *  4. 동작 순서
 *  	  				 .do					request
...
public class DispatcherServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		try {
			URL url=this.getClass().getClassLoader().getResource("."); //현재 폴더 갖고오기
			File file=new File(url.toURI());
			//System.out.println(file.getPath());
			String path=file.getPath();
			path=path.replace("\\", file.separator);// file.sepqrator = Window => \\, Mac => /
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			try {
			String path=request.getRequestURI(); //URL을 받는다 //?는 짤린다 ?= requset에 담아서 보내짐
