    ...
    <%--
    	...
    	1) 지시자
    	   = page
    	     <%@ page 속성=값 ... %> : JSP파일 정보
    	     속성 
    	     	= contentType : 브라우저와 연결 => 파일형태 알려주는 역할
    	     	  text/html => html
    	     	  text/xml  => XML
    	     	  text/plain => JSON
    	...
    	1-1) 스크립트릿
    		 ...
    		 스크립트릿 : <% 일반 자바 : 지역변수, 메소드 호출, 객체생성, 제어문 %>
    	2) 내장객체
    		자바에서 지원
    	...
    	3) JSP 액션 태그
    		...
    		<jsp:setProperty> : set 메소드에 값을 채움
...
<script type="text/javascript">
	let ok=(zip,addr) =>{
		opener.frm.post1.value=zip.substring(0,zip.indexOf("-"));
		opener.frm.post2.value=zip.substring(zip.indexOf("-")+1);
		opener.frm.addr.value=addr;
		self.close();
	}
	
	/* let ok=(zip,addr) =>{
		opener.frm.post1.value=zip;
		opener.frm.addr.value=addr;
		self.close();
	} */
		...
		<div class=row>
			<form method=post action=post.jsp>
				<table class=table>
					<tr>
						<td>
							우편변호 : <input type=text name=dong size=15 class=input-sm>
					...
					<table class=table>
						...
						%>
									<tr>
										...
										<td width=80%>
											<a href="javascript:ok('<%=vo.getZipcode()%>','<%=vo.getAddress()%>')">
												<%=vo.getAddress()%>
											</a>
										</td>
									</tr>
