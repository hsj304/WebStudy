...
//						+ "JOIN myGrade mg "
//						+ "ON me.sal BETWEEN mg.losal AND mg.hisal "
//						+ "ORDER BY empno DESC";
			String sql = "SELECT empno,ename,job,TO_CHAR(hiredate,'YYYY-MM-DD'),TO_CHAR(sal,'L999,999'),NVL(comm,0),dname,loc,grade "
...
//					   + "FROM myEmp me,myDept md,myGrade mg "
//					   + "WHERE me.deptno=md.deptno AND me.sal BETWEEN mg.losal AND hisal "
//					   + "AND empno=?";
			String sql = "SELECT empno,ename,job,NVL(mgr,0),TO_CHAR(hiredate,'YYYY-MM-DD'),TO_CHAR(sal,'L999,999'),NVL(comm,0), "
						+ "(SELECT dname FROM myDept md WHERE md.deptno=me.deptno), "
						+ "(SELECT loc FROM myDept md WHERE md.deptno=me.deptno), "
						+ "(SELECT grade FROM myGrade mg WHERE me.sal BETWEEN mg.losal AND mg.hisal) "
						+ "FROM myEmp me "
						+ "WHERE empno=?";
