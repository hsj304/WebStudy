...
<%---
	...
	3) 내장 객체 : 미리 객체를 생성해서 필요한 위치에서 사용이 가능
		...
		response : 응답 정보, 화면 이동 정보 => HttpServletResponse
				   -------
				   = setContentType() => HTML,XML
				   	=> page지시자
				   = addCookie() : 쿠키 전송
				   -------------- 한번만 수행할 수 있다
				   화면 이동 정보
				   -----------
				   필요시에 서버에서 화면을 요청화면이 아닌 다른화면 이동
				   	      -----------------------------------
				   	      login=>main
				   	      insert => list
				   	      
				   => sendRedirect() => GET방식, reques를 초기화한 다음 이동
...
out.print("<table class=\"table table-stripted\">");
